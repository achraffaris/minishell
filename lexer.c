...
t_token	*get_next_token(t_lexer *lexer)
{
	while(lexer->c != '\0')
	{
		...
		else if(lexer->c != '>' && lexer->c != '|' && lexer->c != '<' && lexer->flg == 1)
		{
			lexer->flg = 0;
			return init_token(TOKEN_CMD, collect_string(lexer));
		}
		else if(lexer->c != '>' && lexer->c != '|' && lexer->c != '<' && lexer->flg == 0)
		...
		else if(lexer->c == '>')
		{
			lexer_advance(lexer);
			if (lexer->c == '>')
				return lexer_advance_with_token(lexer, init_token(TOKEN_DRRD, get_current_char_as_string_redirection(lexer)));
			else
				return lexer_advance_with_token(lexer, init_token(TOKEN_SRRD, get_current_char_as_string_redirection(lexer)));
		}
		else if(lexer->c == '<')
		{
			lexer_advance(lexer);
			if (lexer->c == '<')
				return lexer_advance_with_token(lexer, init_token(TOKEN_HERDOC, get_current_char_as_string_redirection(lexer)));
			else
				return lexer_advance_with_token(lexer, init_token(TOKEN_SLRD, get_current_char_as_string_redirection(lexer)));
		}
		else if(lexer->c == '$')
...
{ 
	...
	while(lexer->c != '\0')
	{
		...
		else if (lexer->c == '$')
		{
			...
			&& lexer->c != '<' && lexer->c != ' ' && lexer->c != '|' && lexer->c != '\0')
		{
			s = get_current_char_as_string(lexer);
			lexer_advance(lexer);
		}
...
char	*get_current_char_as_string_redirection(t_lexer *lexer)
{
	...
	if (lexer->c == ' ')
		lexer_skip_space(lexer);
	if (lexer->c == '>' || lexer->c == '<' || lexer->c == '\0' || lexer->c == '|')
		lexer->flg_error = 1;
...
{
	lexer->flg = 1;
	return lexer_advance_with_token(lexer, init_token(TOKEN_PIPE, get_current_char_as_string(lexer)));
}